...
int main(int argc, char* argv[]) {
    ...
    if (random_chance(5)) {
        printf("Klient %d idzie do restauracji PRZED wejściem do parku\n", g_klient.pidKlienta);
    ...
    wejdz_do_parku();
    if (random_chance(5)) {
        printf("Klient %d idzie do restauracji PO wyjściu z parku\n", g_klient.pidKlienta);
        idz_do_atrakcji(16, g_klient.pidKlienta);
    }
...
void wejdz_do_parku() {
    ...
    if (g_klient.czyVIP) {
        ...
        printf("Klient %d (VIP) wchodzi do kolejki priorytetowej\n", g_klient.pidKlienta);
    } else {
        k_msg.mtype = MSG_TYPE_STANDARD_TICKET;
        printf("Klient %d wchodzi do kolejki (bilet: %s, osób: %d)\n",
    ...
    printf("Klient %d wychodzi z kolejki do kasy\n", g_klient.pidKlienta);
    fflush(stdout);
...
int idz_do_atrakcji(int nr_atrakcji, pid_t identifier) {
    ...
    if (contains(anulowalne, nr_atrakcji) && czyZrezygnuje) {
        ...
        while (curTime <= timeout) {
            ...
            int mess = msgrcv(atrakcja_id, &mes, sizeof(ACKmes) - sizeof(long),
        ...
        printf("Klient %d REZYGNUJE z %s po %d min\n",
...
void wyjdz_z_parku() {
    ...
    printf("Klient %d idzie do kasy zapłacić przy wyjściu\n", g_klient.pidKlienta);
